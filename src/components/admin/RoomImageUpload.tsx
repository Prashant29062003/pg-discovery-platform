...
export function RoomImageUpload({ 
  ...
  const [uploadingImages, setUploadingImagesLocal] = useState(false);
    ...
    try {
      ...
      if (result.success) {
        ...
        if (onImagesChange) {
          ...
          onImagesChange(updatedImages.map(p => ({ id: p.id, url: p.url, name: p.name })));
        }

        showToast.success('Room image uploaded successfully', image.name);
      } else {
  ...
  }, [previewImages, setValue, watch, roomId, onImagesChange]);
    ...
    <Card className={cn('w-full', className)}>
      ...
      <CardContent className="space-y-4">
        ...
        <div className="text-xs text-muted-foreground space-y-1">
          ...
          <p>• First image will be set as the room's primary image</p>
        </div>
      </CardContent>
    </Card>
  );
