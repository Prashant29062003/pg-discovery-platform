...
export default function EnquiriesPage() {
  ...
  const { data: enquiries, pg, loading, error } = usePropertyData({
    pgId,
    dataType: 'enquiries',
  });
  ...
  //         <AlertCircle className="w-12 h-12 text-red-500 mb-2" />
  //       </div>
  //       <h3 className="text-lg font-semibold text-red-700 dark:text-red-400">Error loading enquiries</h3>
            ...
            {enquiries.map((enquiry) => (
              ...
              >
                <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3 mb-3">
                  <div>
                    <h3 className="font-semibold text-zinc-900 dark:text-zinc-50">{enquiry.visitorName}</h3>
                    <p className="text-sm text-zinc-500 dark:text-zinc-400">{enquiry.email} • {enquiry.phone}</p>
                  </div>
                  <span className="text-xs text-zinc-500 dark:text-zinc-400 md:text-right">
                    {new Date(enquiry.createdAt).toLocaleDateString()} {new Date(enquiry.createdAt).toLocaleTimeString()}
                  </span>
                </div>
                <p className="text-sm text-zinc-700 dark:text-zinc-300">{enquiry.message}</p>
              </div>
            ))}
          </div>
        )}
